"""This version uses the same code as v1.0 but increases the values of space and time divisions in a loop to find
optimal parameter values such that the solution has converged to a chosen tolerance.
...
# params = {'lambda1': 0.1290983606557377, 'lambda2': 0.7098591549295775, 'mu1': 0.009445871025530073,
#           'mu2': -0.448658871479901, 'sigma': 0.22223883723618254, 'K': 0.001, 'rho': 0.02144999980926514}
...
M = 100  # number of divisions of the space R
forward_diff = False
backward_diff = False
tol = 10**-3
...
while True:
    ...
    pb_err = abs(pb_boundary_arr[-1][0] - pb_old)
    ps_err = abs(ps_boundary_arr[-1][0] - ps_old)
    print("N={}, M={}, Pb_error={:.10f}, Ps_error={:.10f}".format(N, M, pb_err, ps_err))
    if pb_err <= tol and ps_err <= tol:
        break
    else:
        M = M * 2
        pb_old = pb_boundary_arr[-1][0]
        ps_old = ps_boundary_arr[-1][0]

print("N={}, M={}, Pb={:.10f}, Pb={:.10f}".format(N, M, pb_boundary_arr[-1][0], ps_boundary_arr[-1][0]))
